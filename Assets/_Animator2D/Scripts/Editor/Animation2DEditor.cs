...
namespace EtienneEditor.Animator2D
{
    ...
    public class Animation2DEditor : Editor<Animation2D>
    {
        ...
        private async void Update()
        {
            ...
            while (isEnabled)
            {
                Frame currentFrame = currentTarget.Frames[index % currentTarget.Frames.Length];
                int frameTime = Mathf.RoundToInt(1000 / currentTarget.FPS * currentFrame.Duration);
                await System.Threading.Tasks.Task.Delay(frameTime);
                if (currentTarget.Frames.Length <= 0 || !HasPreviewGUI())
                {
                    OnDisable();
                    break;
                }
                Repaint();
                ++index;
                if (index >= currentTarget.Frames.Length)
                {
                    index = 0;
                    ++targetIndex;
